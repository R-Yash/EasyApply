...
class EasyApplyBot:
    ...
    def __init__(self,
        ...
        self.locator = {
            ...
            "fields": (By.XPATH, "//div[contains(@class,'jobs-easy-apply-form-section__grouping')]"),
    ...
    def start_apply(self, positions, locations,remote) -> None:
    ...
    def applications_loop(self, position, location,remote):
        ...
        self.browser, _ = self.next_jobs_page(position, location, jobs_per_page,remote,experience_level=self.experience_level)
        log.info("Looking for jobs.. Please wait..")
    ...
    def apply_to_job(self, jobID):
        ...
        self.write_to_file(button, "https://www.linkedin.com/jobs/view/" + jobID, self.browser.title, result)
        return result
    ...
    def write_to_file(self, button, jobID, browserTitle, result) -> None:
        ...
        toWrite: list = [jobID, job, company,"Applied" if result else "Not Applied",date,time_]
        ...
        try:
            ...
            while loop < 2:
                ...
                if len(self.get_elements("submit")) > 0:
                    ...
                    if "Application submitted" in self.browser.page_source:
                        log.info("Application Submitted")
                        submitted = True
                        break
                    elif len(elements) > 0:
                        ...
                        while len(elements) > 0:
                            log.info("Please answer the questions, waiting 15 seconds...")
                            time.sleep(15)
                            elements = self.get_elements("error")
    ...
    def ans_question(self, question):
        ...
        for keyword, ans in keywords.items():
            if re.search(re.compile(keyword),question.lower()):
                answer = ans
                break
        ...
        if answer is None:
            ...
            notification.notify(
                title = 'Question cannot be answered automatically',
                message = f'Please Answer the question or apllication will be skipped: {question} ',
                timeout = 60,
            )
            time.sleep(100)
            raise Exception("Not able to answer question automatically. Please provide answer")

        log.info(f"Answering question: '{question}' with answer: '{answer}'")

        # Append question and answer to the CSV if not already present
    ...
    def next_jobs_page(self, position, location, jobs_per_page,remote,experience_level=[]):
        ...
        if remote == False:
            ...
            self.browser.get(
                "https://www.linkedin.com/jobs/search/?f_WT=2&f_LF=f_AL&keywords=" +
                position + location + "&start=" + str(jobs_per_page) + experience_level_param)
...
if __name__ == '__main__':
    ...
    assert parameters['remote'] is not None
